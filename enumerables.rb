...
def print_spiciest_foods(spicy_foods)
  over_five = spicy_foods.filter {|food| food[:heat_level] > 5}
  formatted_foods = over_five.map {|food| "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{'🌶' * food[:heat_level]}"}
  puts formatted_foods
end
...
def average_heat_level(spicy_foods)
  num = 0
  spicy_foods.each {|food| num = num + food[:heat_level]}
  num / 3
end
